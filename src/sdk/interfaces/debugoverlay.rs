...
type WorldToScreenFn = unsafe extern "thiscall" fn(
    thisptr: *mut usize,
    input: *const Vector3<f32>,
    out: *mut Vector3<f32>,
) -> u8;
...
impl CDebugOverlay {
    ...
    pub fn world_to_screen(&self, position: &Vector3<f32>) -> Option<Vector3<f32>> {
        ...
        let return_code = unsafe {
            transmute::<_, WorldToScreenFn>(get_virtual_function(self.base, 11))(
                self.base,
                position as *const _,
                &mut return_vec as *mut _,
            )
        };
